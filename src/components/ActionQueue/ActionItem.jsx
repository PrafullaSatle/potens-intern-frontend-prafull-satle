...
function ActionItem({ item, onApprove, onHold }) {
  ...
  return (
    <div className="flex flex-col gap-3 py-4 md:grid md:grid-cols-[100px_1fr_70px_90px_70px_120px] md:gap-3 md:items-center md:px-3 md:py-4 hover:bg-slate-50 transition">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 md:contents">
        <span className="text-sm text-slate-600">{item.ward}</span>
        <span className="text-sm text-slate-600">{item.reported}</span>
        <span className={`inline-flex justify-center items-center px-2 py-1 rounded-full text-xs font-bold ${getSlaStyle(item.sla)}`}>
          {item.sla}
        </span>
      </div>
      ...
      <div className="flex gap-1.5">
        <button
          ...
          className="rounded-lg bg-emerald-600 px-2.5 py-1.5 text-xs text-white hover:bg-emerald-700 transition"
        >
        ...
        <button
          ...
          className="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs hover:bg-slate-100 transition"
        >
          {t("actionQueue.hold")}
        </button>
      </div>
